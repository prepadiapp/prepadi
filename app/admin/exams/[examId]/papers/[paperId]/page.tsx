...
export default function PaperEditorPage() {
  ...
  // DnD Hydration Fix: Ensure DnD only runs on client
  ...
  useEffect(() => {
    const loadPaper = async () => {
      try {
        ...
        setPaper(data);
        // Sort by existing order or fallback to index
        const sortedQs = (data.questions || []).sort((a: any, b: any) => (a.order || 0) - (b.order || 0));
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    // Get the list of questions for the CURRENT tab only
    ...
    // 1. Map new order to the items in the current tab
    const updatedTabItems = newTabItems.map((q, idx) => ({ ...q, order: idx + 1 }));
    
    // 2. Combine with items from the other tab (untouched)
    const otherTabQuestions = allQuestions.filter(q => q.type !== activeTab);
    const mergedQuestions = [...otherTabQuestions, ...updatedTabItems].sort((a, b) => a.order - b.order);
  ...
  const handleSaveOrder = async () => {
    setSaving(true);
    try {
      // Re-normalize order across the entire set to ensure no gaps
      ...
      const res = await fetch(`/api/admin/papers/${paperId}/reorder`, {
        ...
        body: JSON.stringify({ updates: normalizedQuestions })
      });

      if (!res.ok) throw new Error("Failed to save order");
      
      toast.success("Order saved successfully!");
      setAllQuestions(normalizedQuestions as QuestionItem[]); // Update local state with normalized order
    } catch (e) {
        ...
        <CardHeader className="pb-2 border-b">
            <div className="flex justify-between items-center">
                ...
                <Button variant="secondary" size="sm" asChild>
                    <Link href={`/admin/questions/new?paperId=${paperId}&type=${activeTab}`}>
        ...
        <CardContent className="pt-6 bg-slate-50/50 min-h-[500px]">
            {questionsToShow.length === 0 ? (
                <div className="text-center py-20 text-muted-foreground border-2 border-dashed rounded-lg">
                    <p>No {activeTab.toLowerCase()} questions found.</p>
                    <Button variant="link" asChild><Link href={`/admin/questions/new?paperId=${paperId}`}>Add your first question</Link></Button>
                </div>
            ) : (
                enabled && (
