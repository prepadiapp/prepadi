...
import { GripVertical, Layers, ArrowDownUp } from 'lucide-react';
...
export function PaperEditor({ paperId, initialQuestions }: PaperEditorProps) {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    try {
        ...
        await fetch(`/api/admin/papers/${paperId}/reorder`, {
            method: 'POST',
    ...
    <div className="space-y-4 md:space-y-6">
       ...
       {questions.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 md:py-16 border-2 border-dashed border-slate-200 rounded-lg bg-slate-50/50 text-center px-4">
             ...
             <Button className="mt-4 h-8 text-xs" size="sm" variant="outline">
                Add First Question
             </Button>
          </div>
       )}
